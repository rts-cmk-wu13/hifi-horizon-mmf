...
export default function CardHome() {
  ...
  return (
    <section className="grid grid-rows-[4rem_1fr] grid-cols-[2.313rem_auto_1fr_1fr_auto_2.313rem] items-center justify-items-center">
      <h1
        className="col-start-2 text-[#495464] text-[1.5rem] font-bold
      "
      >
        popular products
      </h1>
      <Link to={"/Shop"} className="col-start-[-3]">
        <button
          className="
      bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 px-4 w-50 h-10 flex justify-center rounded shadow"
        >
          See all products
        </button>
      </Link>
      <div
        className="card--wrapper col-start-2 col-end-[-2] row-start-2 gap-8
      ...
      w-[16.125rem] bg-white rounded-lg shadow-md  p-6 text-center h-[21.188rem]"
          >
            ...
            />
            <p className="text-base text-[0.875] font-bold">
              {product.product_name}
            </p>
            <p className="text-[1.5rem] font-bold my-4">
              DKK {product.price_dkk}
            </p>
